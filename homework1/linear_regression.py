...
def linear_regression_noreg(X, y):
  ...
  """
  w = np.linalg.inv(np.dot(X.transpose(),X)).dot(X.transpose()).dot(y)
  return w
...
def regularized_linear_regression(X, y, lambd):
  """
    ...
    - w: a numpy array of shape (D, )
    """
  w = np.linalg.inv(np.dot(X.transpose(),X) + lambd*np.identity((X.shape)[1])).dot(X.transpose()).dot(y) 
  return w
...
def test_error(w, X, y):
  """
    ...
    """
  err = ((y-np.dot(w, X.transpose()))**2).mean()
  return err
