...
def pca(X = np.array([]), initial_dims = 50):
    ...
    m = np.mean(X, axis=0)
    datac = np.array([obs - m for obs in X])
    ...
    for i in range(N):
        cov_matrix += np.dot(np.expand_dims((X[i] - mean_X),axis=0).T, np.expand_dims(X[i] - mean_X,axis=0))
    eig_val, eig_vec = np.linalg.eig(cov_matrix)
    eig_pairs = [(np.abs(eig_val[i]), eig_vec[:,i]) for i in range(len(eig_val))]
    eig_pairs.sort(key=lambda x: x[0], reverse=True)
    matrix_w  = np.array([eig_pairs[0][1]])
    for i in range(1,initial_dims):
        matrix_w = np.concatenate((matrix_w,[eig_pairs[i][1]]),axis=0)
    Y = datac.dot(matrix_w.T)
...
def compute_Q(Y):
    ...
    Y1 = np.repeat([dot_prod.diagonal()], Y.shape[0], axis=0)
    Y_sim = np.reciprocal(1 + Y1 + Y1.T - 2 * dot_prod)
    Q = np.divide(Y_sim,(np.sum(Y_sim) - Y.shape[0]))
    
    return Q.tolist(), Y_sim.tolist()
...
def compute_gradient(P, Q, Y_sim, Y, dY, no_dims):
    ...
    dot_prod = np.dot(Y,Y.T)
    Y1 = np.repeat([dot_prod.diagonal()], Y.shape[0], axis=0)
    L2_dist_inv = np.reciprocal(1 + Y1 + Y1.T - 2 * dot_prod)
    #for i in range(Y.shape[0]):
    #    for j in range(Y.shape[0]):
    #        dY[i] += 4*(P[i,j]-Q[i,j])*(Y[i]-Y[j])* L2_dist_inv[i,j]
    for i in range(Y.shape[0]):
        dY[i] = np.dot(np.expand_dims(np.multiply(4*(P[i,:]-Q[i,:]),L2_dist_inv[i,:]),axis=0),(np.repeat([Y[i,:]],Y.shape[0],axis=0)-Y))[0]
    return dY
