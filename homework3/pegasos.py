...
def objective_function(X, y, w, lamb):
    ...
    trainSize = X.shape[0]
    obj_value = lamb/2.0*(np.sum(w**2)) + 1.0/trainSize*np.sum(np.maximum(np.zeros((1,trainSize)),np.ones((1,trainSize))-np.multiply(np.expand_dims(y,axis=0),np.dot(w.transpose(), X.transpose()))))
    return obj_value
...
def pegasos_test(Xtest, ytest, w, t = 0.):
    ...
    test_acc = np.mean((2*(np.dot(w.transpose(), Xtest.transpose()) > t)-1) == ytest)
    return test_acc
